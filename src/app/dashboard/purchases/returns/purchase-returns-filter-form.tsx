...
const FILTER_FIELDS = ["from", "to", "supplier", "purchaseNo", "creditNote", "product", "batch", "recordedBy"] as const;
...
export function PurchaseReturnsFilterForm({
  actionPath,
  from,
  to,
  supplier,
  ...
  batch: string;
  recordedBy: string;
  initialOptions: { suppliers: string[]; products: string[]; recordedBy: string[] };
  hiddenLimit?: string;
  clearHref: string;
  ...
  const { options, refresh } = useCascadingFilterOptions(
    initialOptions,
    "/api/dashboard/purchases/returns/filter-options",
    readParams,
  );
  ...
  return (
    ...
    >
      ...
      <label className="flex flex-col gap-1">
        <span className="text-xs font-medium uppercase tracking-wide text-zinc-500">Purchase no.</span>
        <input
          name="purchaseNo"
          type="search"
      ...
      <label className="flex flex-col gap-1">
        <span className="text-xs font-medium uppercase tracking-wide text-zinc-500">Credit note</span>
        <input
          name="creditNote"
          type="search"
