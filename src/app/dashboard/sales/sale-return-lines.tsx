...
function lineReturnState(line: SaleReturnLineApi, raw: string) {
  const q = raw === "" ? 0 : parseInt(raw, 10);
  const lineCredit =
    Number.isFinite(q) && q > 0
      ? returnLineRefundInclusive(line.qty, q, line.amount, line.discountAmount)
      : 0;
...
export function SaleReturnLinesMobile({
  lines,
  qtyByLineId,
  onQtyChange,
}: {
  lines: SaleReturnLineApi[];
          ...
          >
            ...
            <dl className="mt-4 grid grid-cols-2 gap-x-3 gap-y-2 text-sm">
              ...
              <div className="col-span-2">
                <dt className="text-xs text-zinc-500">Return qty</dt>
                <dd className="mt-1">
...
export function SaleReturnLinesTable({
  lines,
  qtyByLineId,
  onQtyChange,
}: {
